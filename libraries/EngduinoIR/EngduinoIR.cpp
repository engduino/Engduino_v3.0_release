...
void EngduinoIRClass::begin()
{
	...
	// data is looped back.
	//
	EICRA &= 0xCF;			// Make sure interrupt sense control bits for INT2 are cleared
	EICRA |= 0x20;			// ...and set them to sense a falling edge
	EIMSK |= 0x04;			// Enable the INT2 interrupt
...
{   
	...
	if (b & 1) {
#if defined(__BOARD_ENGDUINOV1)|| (__BOARD_ENGDUINOV3)
		mark(BITTIME);
#elif defined(__BOARD_ENGDUINOV2)
		mark(MARKTIME);
		mark(MARKTIME);
